...
export class ClaudeAdapter implements ProviderAdapter {
  ...
  canParse(content: string): boolean {
    try {
      ...
      return (
        firstLine.type === "user" ||
        firstLine.type === "assistant" ||
        (firstLine.sessionId !== undefined && firstLine.uuid !== undefined)
      );
    } catch {
      return false;
    }
    ...
    if (filePath) {
      ...
      for (const agent of agents) {
        ...
        if (existsSync(agentFilePath)) {
          const agentData = this.parseAgentTrajectory(agentFilePath, sessionId, agent.agentId);
  ...
  private parseAgentTrajectory(filePath: string, sessionId: string, agentId: string): {
