...
import type { MessageData, ToolCallData, AgentData, SessionData } from "./db-v2";
...
interface AgentTrajectoryData {
  messages: MessageData[];
  toolCalls: ToolCallData[];
}

function parseAgentTrajectory(filePath: string, sessionId: string, agentId: string): AgentTrajectoryData {
  const content = readFileSync(filePath, "utf-8");
  const lines = content.trim().split("\n").filter(l => l.trim());
  const jsonlMessages: JSONLMessage[] = lines.map(line => JSON.parse(line));
